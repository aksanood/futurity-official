...
const ContactForm: React.FC<ContactFormProps> = ({ source = 'homepage', onSuccess }) => {
  ...
  return (
    <>
      ...
      <p className="text-white/80 mb-2">by partnering with Futurity.</p>
      <div className="w-16 h-1 bg-white mb-4" style={{ height: '5px', borderRadius: '2px' }}></div>
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        <div className="relative">
          ...
          <input type="text" name="name" placeholder="Name" value={form.name} onChange={handleChange} required className="w-full bg-white/10 text-white rounded-lg pl-10 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-futurity-orange placeholder-white/60" />
        </div>
        <div className="relative">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-white/70">
            {/* Mail SVG */}
          ...
          <input type="email" name="email" placeholder="Email address" value={form.email} onChange={handleChange} required className="w-full bg-white/10 text-white rounded-lg pl-10 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-futurity-orange placeholder-white/60" />
        </div>
        <div className="relative">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-white/70">
            {/* Call SVG */}
          ...
          <input type="tel" name="phone" placeholder="Telephone number" value={form.phone} onChange={handleChange} className="w-full bg-white/10 text-white rounded-lg pl-10 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-futurity-orange placeholder-white/60" />
        </div>
